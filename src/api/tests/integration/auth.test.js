...
import { CREATED, CONFLICT, BAD_REQUEST, OK, UNAUTHORIZED } from "http-status";
import { expect } from "chai";
import { createSandbox } from "sinon";
import moment from "moment-timezone";
import app from "../../../index";
import { deleteMany, create } from "../../models/user.model";
import {
...
describe("Authentication API", () => {
  ...
  describe("POST /v1/auth/refresh-token", () => {
    ...
    it("should report error when the refreshToken is expired", async () => {
      await _create(expiredRefreshToken);
  ...
  describe("POST /v1/auth/send-password-reset", () => {
    it("should send an email with password reset link when email matches a user", async () => {
      const PasswordResetTokenObj = await __create(resetToken);
    ...
    it("should report error when email doesn't match a user", async () => {
      ...
      return request(app)
        ...
        .then((res) => {
          const { code } = res.body;
          const { message } = res.body;
          expect(code).to.be.equal(UNAUTHORIZED);
  ...
  describe("POST /v1/auth/reset-password", () => {
    ...
    it("should report error when the resetToken is expired", async () => {
      const expiredPasswordResetTokenObj = await __create(expiredResetToken);
